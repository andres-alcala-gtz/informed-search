...
class Node:
    def __init__(self, state:np.array, parent:np.array, move:str, h_cost:int) -> None:
...
def get_children(node:Node, goal:np.array) -> list:
    ...
    for direction in DIRECTIONS.keys():
        ...
        if 0 <= new_y < node.state.shape[0] and 0 <= new_x < node.state.shape[1]:
            ...
            successors.append(Node(new_state, node.state, direction, h(new_state, goal)))
...
# searching algorithm
def greedy(start:np.array, goal:np.array) -> tuple:

    open = {np.array_str(start): Node(start, start, '', h(start, goal))}
    closed = {}
...
if __name__ == '__main__':
    ...
    search, path = greedy(start, goal)

    summary = {}
    summary['Searching Algorithm'] = 'Greedy'
