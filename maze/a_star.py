...
# searching algorithm
def a_star(maze:pyamaze.maze) -> tuple:
    ...
    # costs
    g_cost = {row:float('inf') for row in maze.grid}
    ...
    while not open.empty():
        ...
        for direction in ['N', 'S', 'W', 'E']:
            ...
            if maze.maze_map[current][direction]:
                ...
                f_temp = g_temp + h(successor, goal)

                if f_temp < f_cost[successor]:
                    backward[successor] = current
                    g_cost[successor] = g_temp
...
if __name__ == '__main__':
    ...
    label = textLabel(parentMaze=maze, title='Searching Algorithm', value='A*')
